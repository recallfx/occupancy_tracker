...
async def websocket_handler(request):
    ...
    await ws.send_json(
        {"type": "init", "layout": app["layout"], "state": coordinator.data}
    )
    ...
    async def on_update():
        if ws.closed:
            return
        try:
            await ws.send_json({"type": "state_update", "state": coordinator.data})
...
async def verify_history(request):
    ...
    return web.json_response(
        {
            "passed": result,
            "message": "History verification passed"
            if result
            else "History verification failed - see logs",
        }
    )
