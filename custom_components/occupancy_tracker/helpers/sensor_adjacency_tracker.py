...
class SensorAdjacencyTracker:
    ...
    def __init__(self):
        ...
        self.sensor_to_areas: Dict[str, List[str]] = {}  # sensor_id -> list of area_ids

    def set_sensor_area(self, sensor_id: str, area_id: str) -> None:
        """Map a sensor to its area.
        
        Note: This method is kept for backward compatibility but add_sensor_area
        should be preferred to support multi-area sensors.
        """
        self.add_sensor_area(sensor_id, area_id)

    def add_sensor_area(self, sensor_id: str, area_id: str) -> None:
        """Add an area mapping for a sensor."""
        if sensor_id not in self.sensor_to_areas:
            self.sensor_to_areas[sensor_id] = []
        if area_id not in self.sensor_to_areas[sensor_id]:
            self.sensor_to_areas[sensor_id].append(area_id)
    ...
    def check_adjacent_motion(
        ...
        for adj_sensor_id in adjacent_sensor_ids:
            area_ids = self.sensor_to_areas.get(adj_sensor_id, [])
